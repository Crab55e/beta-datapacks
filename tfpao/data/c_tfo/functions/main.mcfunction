...
execute as @e[type=minecraft:item_frame,tag=s.AutoFireworkMachine] at @s if entity @a[distance=..200] run function c_tfo:events/auto_firework_machine/init
execute as @e[tag=s.AutoFireworkMachine,type=minecraft:item_frame] at @s align xyz positioned ~.5 ~ ~.5 if entity @a[predicate=c_tfo:is_sneaking,distance=...5] run kill @s